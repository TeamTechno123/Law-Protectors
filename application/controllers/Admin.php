...
class Admin extends CI_Controller{
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      if($login == null){
        ...
        foreach ($login as $login){
          $this->session->set_userdata('law_admin_id', $login['admin_id']);
        }
  ...
  public function dashboard(){
    $admin_id = $this->session->userdata('law_admin_id');
    if($admin_id){
  ...
  // Save company Data...
    public function save_company(){
      ...
      if($admin_id){
        ...
        // echo print_r($data);
        $company_id = $this->Admin_Model->save_data('law_company', $data);
        $data2 = array(
    ...
    public function edit_company($company_id){
      $admin_id = $this->session->userdata('law_admin_id');
      if($admin_id){
        $company_info = $this->Admin_Model->get_info('company_id', $company_id, 'law_company');
        if($company_info){
    ...
    public function update_company(){
      ...
      if($admin_id){
        ...
        $this->Admin_Model->update_info('company_id', $company_id, 'law_company', $data);
        header('location:'.base_url().'Admin/company_information_list');
      } else{
        header('location:'.base_url().'Admin');
      }
