...
class Report_Model extends CI_Model{
  ...
  public function application_report_list($from_date,$to_date,$company_id2,$manager_id,$branch_id,$service_id,$status_name,$rc_id,$tc_id){
    ...
    if($manager_id != '' || $rc_id != '' || $tc_id != ''){
      ...
      $this->db->select('application.*,application.application_id as appl_id,branch.*,service.*,law_organization.*,trade.*,copy.*,other.*');
  ...
  public function get_application_report_amt($appl_id,$from_date,$to_date){
    ...
    $this->db->from('law_application as application');
    $this->db->where('application.application_id',$appl_id);
    $this->db->group_by('application.application_id');
    $this->db->where("str_to_date(application.application_date,'%d-%m-%Y') BETWEEN str_to_date('$from_date','%d-%m-%Y') AND str_to_date('$to_date','%d-%m-%Y')");
  ...
  public function get_application_report_amt_clear($appl_id,$from_date,$to_date){
    $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
    $this->db->from('law_application as application');
    $this->db->where('application.application_id',$appl_id);
    $this->db->where('payment.payment_status','Clear');
    $this->db->group_by('application.application_id');
    $this->db->where("str_to_date(application.application_date,'%d-%m-%Y') BETWEEN str_to_date('$from_date','%d-%m-%Y') AND str_to_date('$to_date','%d-%m-%Y')");
    $this->db->join('law_payment as payment','application.application_id = payment.application_id','LEFT');
    $query = $this->db->get();
  ...
  public function get_application_report_amt_unclear($appl_id,$from_date,$to_date){
    $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
    $this->db->from('law_application as application');
    $this->db->where('application.application_id',$appl_id);
    $this->db->where('payment.payment_status','Uncleared');
    $this->db->group_by('application.application_id');
    $this->db->where("str_to_date(application.application_date,'%d-%m-%Y') BETWEEN str_to_date('$from_date','%d-%m-%Y') AND str_to_date('$to_date','%d-%m-%Y')");
    $this->db->join('law_payment as payment','application.application_id = payment.application_id','LEFT');
    $query = $this->db->get();
  ...
  // public function outstanding_branch_wise_report_list_clear($from_date,$to_date,$company_id,$branch_id,$service_id){
  //   $this->db->select('application.*,service.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
  //   $this->db->from('law_application as application');
  //   // $this->db->where('application.company_id',$company_id);
  //   $this->db->where('application.branch_id',$branch_id);
  ...
  // Get Clear Status Amount...
  public function get_target_report_amt_clear($branch_id2,$from_date,$to_date){
    $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
    $this->db->from('law_application as application');
    $this->db->where('application.branch_id',$branch_id2);
    $this->db->where('payment.payment_status','Clear');
  ...
  // Get UnClear Status Amount...
  public function get_target_report_amt_unclear($branch_id2,$from_date,$to_date){
    $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
    $this->db->from('law_application as application');
    $this->db->where('application.branch_id',$branch_id2);
    $this->db->where('payment.payment_status','Uncleared');
...
// User Target Total Amount...
  ...
  // User Target Total Clear Amount...
    public function get_target_report_by_user_amt_clear($user_id,$from_date,$to_date,$roll_id){
      $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
      $this->db->from('law_application as application');
      if($roll_id == 2){
        $this->db->where('application.manager_id',$user_id);
    ...
    // User Target Total Uncleared Amount...
    public function get_target_report_by_user_amt_unclear($user_id,$from_date,$to_date,$roll_id){
      $this->db->select('application.*,SUM(payment.GSTAMOUNT) as GSTAMOUNT,SUM(payment.TOTALAMOUNT) as CONTRACTAMOUNT,SUM(payment.RECEVIEDAMOUNT) as RECEVIEDAMOUNT,SUM(payment.LP_AMOUNT) as LP_AMOUNT,SUM(payment.GOVT_FEES) as GOVT_FEES,SUM(payment.B2B) as B2B,SUM(payment.TDS) as TDS');
      $this->db->from('law_application as application');
      if($roll_id == 2){
        $this->db->where('application.manager_id',$user_id);
